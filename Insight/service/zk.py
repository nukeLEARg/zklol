...
class zk(object):
    def __init__(self, service_module):
        ...
        self._km_preProcess: janus.Queue = (
            None  # raw json, before insertion to database
        )
        self._km_postProcess: janus.Queue = (
            None  # fully finished sqlalchemy objects with names resolved
        )
    ...
    def add_delay(q, other_time, minutes=False):
        try:
            ...
            q.put_nowait(
                ((datetime.datetime.utcnow() - other_time).total_seconds()) / div_s
            )
    ...
    def get_stats(self):
        ...
        km_delay = (
            _tmp_km_delay[0],
            _tmp_km_delay[1] if _tmp_km_delay[1] <= 100 else 99,
        )
    ...
    def _make_km(self, km_json):
        ...
        try:
            if dbRow.tb_kills.make_row(km_json, self.service) is not None:
                dbRow.name_resolver.api_mass_name_resolve(
                    self.service,
                    error_ids_404=self.error_ids_404,
                    error_ids_non404=self.error_ids_non404,
                    exclude_nonentity=True,
                )
    ...
    def debug_simulate(self):
        if self.service.cli_args.debug_km:
            msg = "Starting debug mode.\nStarting KM ID: {}\nForce time to now: {}\nKM Limit: {}\n".format(
                str(self.service.cli_args.debug_km),
                str(self.service.cli_args.force_ctime),
            ...
            try:
                time.sleep(1)
                results = (
                    db.query(dbRow.tb_kills)
                    .filter(dbRow.tb_kills.kill_id >= self.service.cli_args.debug_km)
                    .limit(self.service.cli_args.debug_limit)
                    .all()
                )
                db.close()
                for km in results:
    ...
    async def enrich_package(self, client, package):
        """fetches rich km data from esi"""
        package.pop("killmail", None)
        try:
            href_url = package["zkb"]["href"]
            async with client.get(url=href_url, timeout=10) as href_resp:
                if href_resp.status == 200:
                    full_data = await href_resp.json()
                    # Restore the structure expected by the rest of the app
                    package["killmail"] = full_data
                    return package
                else:
                    print(f"Error fetching ESI href: {href_resp.status} for {href_url}")
                    return None
        except Exception as ex:
            print(f"Exception during enrichment: {ex}")
            return None
        return package
                    ...
                    async with client.get(url=self.zk_stream_url, timeout=45) as resp:
                        if resp.status == 200:
                            ...
                            if package is not None:
                                package = await self.enrich_package(client, package)
                            ...
                            print(
                                "{} - RedisQ zk error code: {}  URL: {}".format(
                                    str(datetime.datetime.utcnow()),
                                    resp.status,
                                    str(self.zk_stream_url),
                                )
                            )
                            print(
                                "Error: {} Headers: {} Body: {} ".format(
                                    resp.status, str(headers), str(body)
                                )
                            )
                            lg.warning(
                                "Error: {} Headers: {} Body: {}".format(
                ...
                while self.run:
                    ...
                    try:
                        async with client.ws_connect(
                            self.url_websocket(), heartbeat=10, receive_timeout=120
                        ) as ws:
                        ...
                        lg.info(
                            "Websocket timeout. Restarting the websocket connection."
                        )
                        await asyncio.sleep(15)
                    except Exception as ex:
        ...
        while True:
            try:
                ...
                km = await loop.run_in_executor(
                    zk_thread_pool, partial(self._make_km, json_data)
                )
                if km is not None:
                    await self._km_postProcess.async_q.put(km)
                    InsightLogger.InsightLogger.time_log(
                        lg, st, "JSON parse km_id: {}".format(km.kill_id), 3500
                    )
            except Exception as ex:
                print(ex)
        ...
        while True:
            try:
                ...
                await loop.run_in_executor(
                    zk_thread_pool, partial(self.service.channel_manager.send_km, km)
                )
                InsightLogger.InsightLogger.time_log(
                    lg, st, "Filer pass km_id: {}".format(km.kill_id), 2500
                )
